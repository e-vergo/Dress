...
Also creates symlinks in a flat nodes/ directory for \inputleannode lookup. -/

/-- Header comment for module.tex files. -/
def moduleHeaderPreamble : String :=
  "%%% This file is automatically generated by Dress. %%%"

/-- Get the flat nodes directory path.
    Returns `.lake/build/dressed/nodes/` -/
def getNodesDir (buildDir : System.FilePath) : System.FilePath :=
  buildDir / "dressed" / "nodes"

/-- Get the path for a node symlink in the flat nodes directory.
    Returns `.lake/build/dressed/nodes/{sanitized-label}/` -/
def getNodeSymlinkDir (buildDir : System.FilePath) (label : String) : System.FilePath :=
  getNodesDir buildDir / sanitizeLabel label
...
module_facet blueprint (mod : Module) : FilePath := do
  ...
  dressedJob.mapM fun _moduleJsonPath => do
    ...
    -- Create flat nodes/ directory structure for \inputleannode lookup
    let nodesDir := getNodesDir buildDir
    IO.FS.createDirAll nodesDir

    for label in declLabels do
      let srcDir := moduleDressedDir / sanitizeLabel label
      let dstDir := getNodeSymlinkDir buildDir label
      -- Create the node directory if it doesn't exist
      IO.FS.createDirAll dstDir
      -- Copy decl.tex to the flat nodes directory
      let srcTex := srcDir / "decl.tex"
      let dstTex := dstDir / "decl.tex"
      if ← srcTex.pathExists then
        let content ← IO.FS.readFile srcTex
        IO.FS.writeFile dstTex content
...
/-- Build all module blueprint facets and generate a library index file.
    ...
    and defines macros for `\inputleanmodule` and `\inputleannode`.
...
def buildLibraryBlueprint (lib : LeanLib) : FetchM (Job Unit) := do
  ...
  moduleJobs.mapM fun _ => do
    ...
    -- We use \def with @-naming that plastex can handle
    let modulePathDefs := mods.map fun mod =>
      ...
      -- Define path lookup: \leanmodulepath@Module.Name -> path
    ...
    -- NOTE: These macros use \csname to look up paths, then \input the result
    -- plasTeX handles path storage better than content storage
    let preamble := s!"%%% Library index for {lib.name} - generated by Dress %%%

%%% Macro definitions %%%
\\makeatletter

% \\inputleannode\{label} - inputs a Lean node declaration from flat nodes directory
\\providecommand\{\\inputleannode}[1]\{%
  \\input\{../../.lake/build/dressed/nodes/#1/decl}}
