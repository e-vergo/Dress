...
The seven-status color model tracks the formalization progress of each node:
...
\begin{itemize}
\item \texttt{notReady} (vivid orange, \texttt{\#E8820C}): The declaration has no Lean code or is
  explicitly marked as not ready.
\item \texttt{wip} (teal, \texttt{\#0097A7}): Manually marked as work-in-progress.
\item \texttt{sorry} (red, \texttt{\#C62828}): The proof exists but contains \texttt{sorryAx}.
\item \texttt{proven} (light green, \texttt{\#66BB6A}): Complete proof without sorry.
\item \texttt{fullyProven} (deep forest green, \texttt{\#1B5E20}): Proven, and all transitive
  dependencies are also proven. Computed post-graph-construction.
\item \texttt{axiom} (purple, \texttt{\#7E57C2}): Intentionally unproven axiom.
\item \texttt{mathlibReady} (blue, \texttt{\#42A5F5}): Manually marked as ready for upstream
...
The priority order is: \texttt{mathlibReady} $>$ \texttt{wip} $>$
...
\texttt{LayoutGraph} into SVG output. This is where the seven-status color
...
vivid orange for unstarted, deep teal for wip, vivid red for sorry,
...
(sorry, fully proven, wip) and dark gray otherwise.
...
for the paper variant, collapsing the seven-status model into three categories:
``Verified'' (proven, fully proven, mathlib ready), ``In Progress'' (sorry,
wip), and ``Not Started'' (not ready).
...
The seven-status color model (Chapter~\ref{ch:graph-svg}) was designed to
