...
def BlueprintContent.toLatex : BlueprintContent → CoreM String
  | .node n => return n.latexLabel  -- Just return the label; path is added by caller
...
private def moduleToLatexOutputAux (_module : Name) (contents : Array BlueprintContent) : CoreM Architect.LatexOutput := do
  ...
  let artifacts : Array Architect.LatexArtifact := ← contents'.filterMapM fun
    ...
    | _ => pure none
  -- Header file: direct \input{} calls for each artifact
  let preamble ← latexPreamble
  let header (artifactsDir : System.FilePath) : String :=
    preamble ++ "\n\n" ++
      "\n\n".intercalate (artifacts.map fun ⟨id, _⟩ => Architect.Latex.input (artifactsDir / id)).toList
