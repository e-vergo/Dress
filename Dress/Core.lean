...
This module contains the core types for Dress, including `NodeWithPos` which extends
`Architect.Node` with position and highlighting information.
-/
...
/-- Extended node with position and highlighting information.
    This extends `Architect.Node` with data captured during elaboration. -/
structure NodeWithPos extends Architect.Node where
  ...
  highlightedCode : Option Highlighted := none
  /-- SubVerso highlighted code for just the signature (up to and including `:=`). -/
  highlightedSignature : Option Highlighted := none
  /-- SubVerso highlighted code for just the proof body (after `:=`). -/
  highlightedProofBody : Option Highlighted := none
