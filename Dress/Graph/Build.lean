...
/-- Build a dependency graph from Dress blueprint nodes (without inferred uses).
    Since NodePart no longer carries explicit use/exclude labels, this creates
    nodes with no edges. Use `fromEnvironment` for dependency-aware graphs. -/
...
/-- Override envType on a Dress.NodeWithPos based on ConstantInfo from the environment.
    Detects axiom, class, structure, instance, and abbrev declarations. -/
private def overrideEnvType (env : Lean.Environment) (name : Lean.Name)
    (dressNode : Dress.NodeWithPos) : Dress.NodeWithPos :=
  ...
  | some (.axiomInfo _) =>
    ...
    { dressNode with toNode := updatedNode }
  | some (.inductInfo _) =>
    let kind := if Lean.isClass env name then some "class"
      else if Lean.isStructure env name then some "structure"
      else none
    match kind with
    | some k =>
    ...
    | none => dressNode
  | some (.defnInfo dv) =>
    let kind := if Lean.Meta.isInstanceCore env name then some "instance"
...
/-- Build graph from the environment's blueprint extension and all project-local declarations.
    All eligible project-local declarations are included as graph nodes.
    Tagged (@[blueprint]) declarations get enriched metadata; untagged ones get minimal nodes.
    `projectModules` specifies which modules are considered project-local. -/
def fromEnvironment (env : Lean.Environment) (projectModules : Array Lean.Name)
    : Lean.CoreM Graph := do
  -- Step 1: Compute eligibleNames (all project-local eligible constants)
  let mut eligibleNames : Lean.NameSet := {}
  for h : idx in [:env.header.moduleData.size] do
    ...
    for name in moduleData.constNames do
      ...
      eligibleNames := eligibleNames.insert name
  -- Also insert all blueprintExt names (they may come from non-project modules)
  let mut allBlueprintEntries : Array (Lean.Name × Architect.Node) := #[]
  for i in [:env.allImportedModuleNames.size] do
    allBlueprintEntries := allBlueprintEntries ++ Architect.blueprintExt.getModuleEntries env i
  allBlueprintEntries := allBlueprintEntries ++ Architect.blueprintExt.getEntries env
  for (bpName, _) in allBlueprintEntries do
    eligibleNames := eligibleNames.insert bpName

  -- Step 2: Build resolveLabel : Name → Option String
  -- Tagged names use their latexLabel; untagged eligible names use Name.toString
  let resolveLabel : Lean.Name → Option String := fun name =>
    match Architect.blueprintExt.find? env name with
    | some node => some node.latexLabel
    | none => if eligibleNames.contains name then some name.toString else none

  -- Step 3: Build a set of tagged names for deduplication
  let mut taggedNames : Lean.NameSet := {}
  for (bpName, _) in allBlueprintEntries do
    taggedNames := taggedNames.insert bpName

  -- Step 4: Build NodeBuildData for each eligible name
  let mut nodesData : Array Builder.NodeBuildData := #[]

  -- 4a. Tagged nodes (from blueprintExt) — enriched path
  for (_, node) in allBlueprintEntries do
    let mut dressNode ← Dress.toDressNodeWithPos node
    dressNode := overrideEnvType env node.name dressNode
    let (statementUses, proofUses) ← node.inferUses eligibleNames resolveLabel
    nodesData := nodesData.push {
      node := dressNode
  ...
  -- 4b. Untagged nodes — minimal path
  for name in eligibleNames do
    if taggedNames.contains name then continue  -- already handled above
    let some ci := env.find? name | continue
    let kind := constantKind env name ci
    -- Create minimal Architect.Node
    let hasProof := match ci with
      | .thmInfo _ => true
      | _ => false
    let minimalNode : Architect.Node := {
      name := name
      latexLabel := name.toString
      statement := { text := "", latexEnv := kind }
      proof := if hasProof then some { text := "", latexEnv := "proof" } else none
      discussion := none
      title := none
    }
    let mut dressNode ← Dress.toDressNodeWithPos minimalNode
    dressNode := overrideEnvType env name dressNode
    let (statementUses, proofUses) ← minimalNode.inferUses eligibleNames resolveLabel
    nodesData := nodesData.push {
      node := dressNode
      hasSorry := !proofUses.leanOk
      statementUses := statementUses.uses
...
/-- Compute blueprint coverage for project-local declarations.
    ...
    Declarations are grouped by source position `(module, line)`. Auto-generated
    declarations (from `@[simps]`, `@[to_additive]`, etc.) that share a source line
    with their parent count as one declaration group — if any member is covered,
    the group is covered. Declarations without `DeclarationRanges` are excluded. -/
def computeCoverage (env : Lean.Environment) (projectModules : Array Lean.Name)
    : Lean.CoreM CoverageResult := do
  -- Pass 1: Collect eligible declarations with source positions
  -- Key: (moduleName, line), Value: (isCovered, representative UncoveredDecl)
  let mut groups : Std.HashMap (String × Nat) (Bool × UncoveredDecl) := {}
  let mut noRangeCount : Nat := 0
  ...
  for h : idx in [:env.header.moduleData.size] do
    ...
    for name in moduleData.constNames do
      ...
      -- Skip declarations without source ranges (compiler artifacts)
      let some ranges ← Lean.findDeclarationRanges? name | do
      ...
      | some (existingCovered, existing) =>
        -- Merge: covered if any member is covered; prefer theorem/lemma as representative
        let mergedCovered := existingCovered || isCovered
        let isThm := decl.kind == "theorem" || decl.kind == "lemma"
        let existIsThm := existing.kind == "theorem" || existing.kind == "lemma"
        let representative := if isThm && !existIsThm then decl else existing
        groups := groups.insert key (mergedCovered, representative)
      | none =>
        groups := groups.insert key (isCovered, decl)
  ...
  for (_, covered, representative) in groups do
    totalDeclarations := totalDeclarations + 1
    if covered then
      coveredDeclarations := coveredDeclarations + 1
    else
      uncovered := uncovered.push representative

  if noRangeCount > 0 then
    IO.eprintln s!"  Coverage: excluded {noRangeCount} declarations without source ranges"
