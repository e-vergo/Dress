...
private def isAutoGeneratedComponent (s : String) : Bool :=
...
private def isAutoGenerated (name : Lean.Name) : Bool :=
...
private def isProjection (env : Lean.Environment) (name : Lean.Name) : Bool :=
  env.getProjectionFnInfo? name |>.isSome

/-- Check if a ConstantInfo is an eligible declaration for coverage counting -/
private def isEligibleConstant (ci : Lean.ConstantInfo) : Bool :=
  match ci with
...
private def isProjectModule (modName : Lean.Name) (projectModules : Array Lean.Name) : Bool :=
